...
model=generator()
model.load_state_dict(torch.load(path_experiment+"generator_param_FP.pkl" , map_location='cpu'))
...
for idx, (imgs,masks) in tqdm.tqdm(enumerate(test_loader)):
    ...
    predictions[idx * batch_size:idx * batch_size + mini_batch, :, :, :] = outputs.detach().numpy()
...
for idx, (imgs,masks) in tqdm.tqdm(enumerate(test_loader)):
    imgs=imgs.cuda()
    mini_batch=imgs.size()[0]
    predictions[idx*32:idx*32+mini_batch]=model(imgs)
'''
...
#pred_patches = pred_to_imgs(predictions, patch_height, patch_width, "original")##threshold
